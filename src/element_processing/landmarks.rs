...
/// Estrutura Mestra de Intercepta��o de Marcos Urbanos (Landmarks) de Bras�lia.
/// Retorna `true` se o edif�cio for reconhecido e gerado por este m�dulo.
/// Retorna `false` para devolver o controle ao gerador procedural padr�o.
pub fn generate_unique_landmark(
    ...
    // =====================================================
    // 1. EIXO MONUMENTAL E PAL�CIOS (Niemeyer/Lucio Costa)
    // =====================================================

    if name.contains("congresso nacional")
        || name.contains("c�mara dos deputados")
    ...
    if name.contains("pal�cio do planalto") || name.contains("palacio do planalto") {
    ...
    if name.contains("itamaraty") || name.contains("pal�cio dos arcos") {
        generate_itamaraty(editor, element, ground_y);
        return true;
    }

    if name.contains("justi�a") && name.contains("pal�cio") {
    ...
    if name.contains("museu nacional") && name.contains("rep�blica") {
        generate_museu_nacional(editor, element, ground_y);
        return true;
    }

    if name.contains("teatro nacional") || name.contains("cl�udio santoro") {
    ...
    // =====================================================
    // 3. SETOR BANC�RIO (Colossos Brutalistas)
    // =====================================================
    ...
    if (name.contains("banco de bras�lia") || name.contains("brb")) && name.contains("sede") {
        generate_sede_brb(editor, element, ground_y);
        return true;
    }

    // =====================================================
    // 4. SETOR DE HOT�IS E COM�RCIO
    // =====================================================
    ...
    if is_station && (name.contains("guar�") || name.contains("guara")) {
        generate_estacao_guara(editor, element, ground_y);
        return true;
    }

    if is_station && (name.contains("�guas claras") || name.contains("aguas claras")) {
...
// ============================================================================
// HELPER GEOM�TRICO (Rigor de Rota��o e Orienta��o)
// ============================================================================
...
fn get_oriented_bounds(element: &ProcessedWay) -> OrientedBounds {
    ...
    // Alarga a base 2D a partir do centro com prote��o de precis�o
...
/// aplicando a matriz de rota��o descoberta do mapa real.
#[inline(always)]
...
// ============================================================================
// MATEM�TICA DOS MONUMENTOS (Rigor Escala Real e Matriz de Rota��o)
// ============================================================================
...
fn generate_congresso(editor: &mut WorldEditor, element: &ProcessedWay, ground_y: i32) {
    ...
    // 1. Laje Monumental (Baseada no pol�gono real, independente de rota��o)
    ...
    for y in (ground_y + 4)..=(ground_y + tower_height) {
        for lx in -rx..=rx {
            for lz in -rz..=rz {
                ...
                // Fachadas de vidro na frente/tr�s, concreto cego nos lados
                if lz == -rz || lz == rz {
    ...
    // 3. C�pula Convexa (Senado) - Calota Esf�rica Cl�ssica
    ...
    for lx in (senado_lx - radius)..=(senado_lx + radius) {
        for lz in -radius..=radius {
            ...
            if dist_sq <= radius * radius {
                // BESM-6 Tweak: Prote��o Math pura .max(0.0)
    ...
    // 4. C�pula C�ncava (C�mara) - Bacia/Par�bola Invertida ( y = x� + z� )
    ...
    for lx in (camara_lx - radius_camara)..=(camara_lx + radius_camara) {
        for lz in -radius_camara..=radius_camara {
            ...
            if dist_sq <= radius_camara * radius_camara {
                ...
                let h = (norm_dist * norm_dist * max_h as f64) as i32; // Par�bola pura
