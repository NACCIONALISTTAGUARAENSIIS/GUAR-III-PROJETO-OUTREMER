...
fn detect_bairro(element: &crate::osm_parser::ProcessedWay) -> Bairro {
    let suburb = element.tags.get("addr:suburb")
        .unwrap_or(&"".to_string())
        .to_lowercase();

    if suburb.contains("sqs") { Bairro::SQS }
    else if suburb.contains("sqn") { Bairro::SQN }
    else if suburb.contains("aguas claras") { Bairro::AguasClaras }
    else if suburb.contains("guará") || suburb.contains("guara") { Bairro::Guara }
    else if suburb.contains("samambaia") { Bairro::Samambaia }
    else if suburb.contains("comercial") || suburb.contains("setor de autarquias") { Bairro::Comercial }
    else if suburb.contains("condom") { Bairro::Condominio }
    else { Bairro::Outro }
}

fn detect_tipologia(element: &crate::osm_parser::ProcessedWay) -> Tipologia {
    let amenity = element.tags.get("amenity").map(|s| s.as_str()).unwrap_or("");
    let building = element.tags.get("building").map(|s| s.as_str()).unwrap_or("");
    let railway = element.tags.get("railway").map(|s| s.as_str()).unwrap_or("");
    let station = element.tags.get("station").map(|s| s.as_str()).unwrap_or("");
    let location = element.tags.get("location").map(|s| s.as_str()).unwrap_or("");
    let layer = element.tags.get("layer").map_or(0, |l| l.parse::<i32>().unwrap_or(0));
...
fn detect_edificio_especifico(element: &crate::osm_parser::ProcessedWay) -> EdificioBrasilia {
    ...
    if name.contains("congresso nacional") || name.contains("câmara dos deputados") || name.contains("senado federal") {
    ...
    } else if name.contains("hospital de base") || name.contains("santa lúcia") || name.contains("santa lucia") {
        EdificioBrasilia::HospitalBase
    } else if name.contains("banco do brasil") || name.contains("caixa econômica") || name.contains("banco central") || name.contains("brb") {
...
fn generate_laje(
    ...
    y: i32, offset: i32, has_core: bool, is_atrium: bool
) {
    if max_x - min_x < 3 || max_z - min_z < 3 { return; }
    ...
    for x in (min_x + 1)..(max_x - 1) {
        for z in (min_z + 1)..(max_z - 1) {
            ...
            if valid_void && x >= void_min_x && x <= void_max_x && z >= void_min_z && z <= void_max_z {
                continue;
            }

            // Geometria relativa ao prédio (x - min_x)
            let is_beam = (x - min_x) % grid_span == 0 || (z - min_z) % grid_span == 0;
            let bloco = if is_beam { SMOOTH_STONE } else { POLISHED_ANDESITE };
...
fn generate_elevator_core(
    editor: &mut WorldEditor,
    min_x: i32, max_x: i32, min_z: i32, max_z: i32,
    floor_y: i32, ceiling: i32, offset: i32,
    tipologia: Tipologia
) {
    let largura = max_x - min_x;
    ...
    let core_x_max = if tipologia == Tipologia::Hospital || tipologia == Tipologia::Corporativo { cx + 3 } else { cx + 2 };

    for x in cx - 1..=core_x_max {
        for z in cz - 1..=cz + 2 {
            for y in floor_y..ceiling {

                let is_air = ((x == cx || x == cx + 1 || (x == cx + 2 && core_x_max > cx + 2)) && (z == cz || z == cz + 1))
                    || (z == cz + 2 && (x == cx || x == cx + 1));
...
fn generate_corridor(
    ...
    min_x: i32, max_x: i32, min_z: i32, max_z: i32,
    floor_y: i32, ceiling: i32, offset: i32,
    bairro: Bairro, tipologia: Tipologia
) {
    let largura = max_x - min_x;
    if max_z - min_z < 5 { return; }
    ...
    for x in (min_x + 1)..(max_x - 1) {
        for y in floor_y..ceiling {
            ...
            if (largura as f64 / H_SCALE) >= 6.0 {
                let wall_block = if tipologia == Tipologia::Hospital { SMOOTH_QUARTZ } else { WHITE_CONCRETE };
        ...
        for z in (min_z + 1)..(max_z - 1) {
            ...
            if is_stair_zone {
                for step in 0..=(mez_y - plat_y) {
                    let step_x = if is_x_axis { if x < cx { x + step } else { x - step } } else { x };
                    let step_z = if !is_x_axis { if z < cz { z + step } else { z - step } } else { z };

                    if step_x > min_x && step_x < max_x && step_z > min_z && step_z < max_z {
                        editor.set_block_absolute(STONE_STAIRS, step_x, mez_y - step + offset, step_z, None, None);
                        editor.set_block_absolute(AIR, step_x, mez_y - step + 1 + offset, step_z, None, None);
                        editor.set_block_absolute(AIR, step_x, mez_y - step + 2 + offset, step_z, None, None);
                    }
                }
            }
            ...
            if !is_track_pit {
                ...
                let is_tactile_edge = if is_x_axis { z == cz - 4 || z == cz + 4 } else { x == cx - 4 || x == cx + 4 };
                if is_tactile_edge {
            ...
            let is_pillar = if is_x_axis { (x - min_x) % 8 == 0 && (z == cz - 4 || z == cz + 4) } else { (z - min_z) % 8 == 0 && (x == cx - 4 || x == cx + 4) };
            if is_pillar {
                for h in plat_y..=ground_y {
                    editor.set_block_absolute(SMOOTH_STONE, x, h + offset, z, None, None);
                }
    ...
    for x in min_x..=max_x {
        for z in min_z..=max_z {
            ...
            if x > min_x && x < max_x && z > min_z && z < max_z {
                ...
                let is_track_pit = if is_x_axis { z >= cz - 3 && z <= cz + 3 } else { x >= cx - 3 && x <= cx + 3 };
                if is_track_pit {
                    editor.set_block_absolute(GRAVEL, x, plat_y - 1 + offset, z, None, None);
                } else {
                    editor.set_block_absolute(POLISHED_DIORITE, x, plat_y + offset, z, None, None);
                    let is_tactile = if is_x_axis { z == cz - 4 || z == cz + 4 } else { x == cx - 4 || x == cx + 4 };
                    if is_tactile {
                        editor.set_block_absolute(YELLOW_CONCRETE, x, plat_y + offset, z, None, None);
                    }
                }
...
fn generate_church_layout(
    editor: &mut WorldEditor,
    min_x: i32, max_x: i32, min_z: i32, max_z: i32,
    y: i32, ceiling: i32, offset: i32,
    is_catedral: bool
) {
    let largura = max_x - min_x;
    let prof = max_z - min_z;

    if largura < 7 || prof < 7 { return; }
    ...
    if is_catedral {
        for dx in -3i32..=3i32 {
            for dz in -3i32..=3i32 {
                ...
                if dist <= 9 {
                    ...
                    editor.set_block_absolute(SMOOTH_STONE_SLAB, cx + dx, y + offset, cz + dz, None, None);
                }
            }
        }
        ...
        if is_z_axis {
            ...
            for x in (min_x + 1)..(max_x - 1) {
                for h in y..(ceiling - 2) {
                    editor.set_block_absolute(WHITE_CONCRETE, x, h + offset, altar_z + 2, None, None);
                }
            }
            ...
            for z in (narthex_z + 1)..altar_z {
                ...
                if (z - min_z) % 4 == 0 {
                    for h in y..ceiling {
                        editor.set_block_absolute(SMOOTH_QUARTZ, min_x + 3, h + offset, z, None, None);
                        editor.set_block_absolute(SMOOTH_QUARTZ, max_x - 3, h + offset, z, None, None);
                    }
                }
            ...
            for z in (min_z + 1)..(max_z - 1) {
                for h in y..(ceiling - 2) {
                    editor.set_block_absolute(WHITE_CONCRETE, altar_x + 2, h + offset, z, None, None);
                }
            }
            ...
            for x in (narthex_x + 1)..altar_x {
                ...
                if (x - min_x) % 4 == 0 {
                    for h in y..ceiling {
                        editor.set_block_absolute(SMOOTH_QUARTZ, x, h + offset, min_z + 3, None, None);
                        editor.set_block_absolute(SMOOTH_QUARTZ, x, h + offset, max_z - 3, None, None);
                    }
                }
...
fn generate_monumental_atrium_layout(
    editor: &mut WorldEditor,
    min_x: i32, max_x: i32, min_z: i32, max_z: i32,
    y: i32, ceiling: i32, offset: i32, has_core: bool
) {
    let largura = max_x - min_x;
    let prof = max_z - min_z;
    ...
    if void_max_x - void_min_x < 2 || void_max_z - void_min_z < 2 { return; }
    ...
    for x in (min_x + 1)..(max_x - 1) {
        for z in (min_z + 1)..(max_z - 1) {
            ...
            let is_void_zone = x >= void_min_x && x <= void_max_x && z >= void_min_z && z <= void_max_z;
            ...
            if is_void_zone {
                ...
                let is_void_edge = (x == void_min_x - 1 || x == void_max_x + 1) && z >= void_min_z - 1 && z <= void_max_z + 1
                    || (z == void_min_z - 1 || z == void_max_z + 1) && x >= void_min_x - 1 && x <= void_max_x + 1;
...
fn generate_shopping_layout(
    editor: &mut WorldEditor,
    min_x: i32, max_x: i32, min_z: i32, max_z: i32,
    y: i32, ceiling: i32, offset: i32, has_core: bool, start_y: i32
) {
    generate_monumental_atrium_layout(editor, min_x, max_x, min_z, max_z, y, ceiling, offset, has_core);
    ...
    for x in (min_x + 1)..(max_x - 1) {
        for z in (min_z + 1)..(max_z - 1) {
            ...
            let is_gallery_edge = (x == gallery_min_x || x == gallery_max_x) && z >= gallery_min_z && z <= gallery_max_z
                || (z == gallery_min_z || z == gallery_max_z) && x >= gallery_min_x && x <= gallery_max_x;
...
fn generate_banco_sede_layout(
    ...
    for x in (min_x + 1)..(max_x - 1) {
        if (x - min_x) % modulo == 0 {
            for z in (min_z + 1)..(max_z - 1) {
                for h in y..ceiling {
                    let block = if h == y + 2 { GLASS_PANE } else { LIGHT_GRAY_TERRACOTTA };
                    editor.set_block_absolute(block, x, h + offset, z, None, None);
                }
            }
        }
...
fn generate_residential_layout(
    editor: &mut WorldEditor,
    min_x: i32, max_x: i32, min_z: i32, max_z: i32,
    y: i32, ceiling: i32, offset: i32, bairro: Bairro, total_floors: usize
) {
    ...
    if largura_m < 4.5 || prof_m < 4.5 { return; }
    ...
    if largura_m < 6.0 {
        ...
        for dx in 1i32..=2i32 {
            for dz in 1i32..=3i32 {
                for h in y..forro_y {
                    ...
                    editor.set_block_absolute(block, min_x + 3 + dx, h + offset, meio_z + dz - 1, None, None);
                }
            }
        }
        ...
        for dx in 0i32..=2i32 {
            for dz in 0i32..=3i32 {
                for h in y..forro_y {
                    ...
                    if is_wall {
                        editor.set_block_absolute(WHITE_CONCRETE, min_x + 3 + dx, h + offset, min_z + dz, None, None);
                    }
                }
            }
        }
        ...
        for i in 1..3 {
            ...
            editor.set_block_absolute(OAK_DOOR, divisao + 1, y + 1 + offset, meio_z + 1, None, None);
        }
        ...
        for dx in 0i32..=3i32 {
            for dz in 0i32..=3i32 {
                for h in y..forro_y {
                    ...
                    if is_wall {
                        editor.set_block_absolute(WHITE_CONCRETE, suite_x + dx, h + offset, max_z - dz - 1, None, None);
                    }
                }
            }
        }
                ...
                for dz in 0i32..=4i32 {
                    if kitchen_z + dz < suite_z_bound {
                        for h in y..forro_y {
                            ...
                            if is_wall {
                                editor.set_block_absolute(WHITE_CONCRETE, kitchen_x_start + dx, h + offset, kitchen_z + dz, None, None);
                            }
                        }
                    }
                }
            }
            editor.set_block_absolute(OAK_DOOR, kitchen_x_start + 1, y + 1 + offset, kitchen_z + 2, None, None);
            editor.set_block_absolute(POLISHED_ANDESITE, max_x - 2, y + 1 + offset, kitchen_z + 1, None, None);
    ...
    if total_floors > 1 && alt_andar >= 4 && prof_m > 8.0 {
        ...
        if escada_x + 3 < max_x && escada_z + 5 < max_z {
            ...
            for i in 0..degraus_lance1 {
                if escada_z + i < max_z {
                    editor.set_block_absolute(STONE_STAIRS, escada_x, y + i + offset, escada_z + i, None, None);
                    editor.set_block_absolute(STONE_STAIRS, escada_x + 1, y + i + offset, escada_z + i, None, None);
                    editor.set_block_absolute(GLASS_PANE, escada_x + 2, y + i + 1 + offset, escada_z + i, None, None);

                    for h in y..(y + i) {
                        editor.set_block_absolute(STONE_BRICKS, escada_x, h + offset, escada_z + i, None, None);
                        editor.set_block_absolute(STONE_BRICKS, escada_x + 1, h + offset, escada_z + i, None, None);
                    }
                }
            }
            ...
            if escada_z + degraus_lance1 + 1 < max_z {
                ...
                editor.set_block_absolute(GLASS_PANE, escada_x + 2, patamar_y + 1 + offset, escada_z + degraus_lance1, None, None);
            }
            ...
            for i in 0..degraus_lance2 {
                ...
                if z_pos >= min_z {
                    editor.set_block_absolute(STONE_STAIRS, escada_x + 1, patamar_y + i + offset, z_pos, None, None);
                    editor.set_block_absolute(GLASS_PANE, escada_x, patamar_y + i + 1 + offset, z_pos, None, None);

                    for h in y..(patamar_y + i) {
                        editor.set_block_absolute(STONE_BRICKS, escada_x + 1, h + offset, z_pos, None, None);
                    }
                }
            }
        }
...
fn generate_school_layout(
    ...
    if largura_interna < tamanho_sala * 2 { return; }
...
pub fn generate_building_interior(
    ...
    if tipologia == Tipologia::Religioso {
        ...
        generate_church_layout(editor, min_x, max_x, min_z, max_z, floors[0] + 1, ceiling, offset, is_catedral);
        return;
    }
    ...
    for i in 0..total_floors {
        ...
        let is_tipologia_vertical = tipologia == Tipologia::Hospital || tipologia == Tipologia::Corporativo || tipologia == Tipologia::Comercial;
        let has_core = (total_floors > 2) || (height > 12 && is_tipologia_vertical);

        let is_atrium = matches!(edificio_unico, EdificioBrasilia::CongressoNacional | EdificioBrasilia::STF | EdificioBrasilia::PalacioPlanalto | EdificioBrasilia::Itamaraty | EdificioBrasilia::Shopping);

        if i > 0 {
            generate_laje(editor, min_x, max_x, min_z, max_z, floor_y, offset, has_core, is_atrium);
        }

        match edificio_unico {
            EdificioBrasilia::CongressoNacional | EdificioBrasilia::STF | EdificioBrasilia::PalacioPlanalto | EdificioBrasilia::Itamaraty => {
                generate_monumental_atrium_layout(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, has_core);
                if has_core { generate_elevator_core(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, tipologia); }
            }
            EdificioBrasilia::Shopping => {
                generate_shopping_layout(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, has_core, start_y);
                if has_core { generate_elevator_core(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, tipologia); }
            }
            EdificioBrasilia::HospitalBase => {
                // Design Exclusivo do Hospital de Base (Maior da América Latina)
                generate_hospital_base_layout(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, has_core);
                if has_core { generate_elevator_core(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, tipologia); }
            }
            EdificioBrasilia::SedeBancaria => {
                generate_banco_sede_layout(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset);
                if has_core { generate_elevator_core(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, tipologia); }
            }
            EdificioBrasilia::CatedralMetropolitana => {}
            EdificioBrasilia::Nenhum => {
                match tipologia {
                    Tipologia::Escola => {
                        generate_corridor(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, bairro, tipologia);
                        generate_school_layout(editor, min_x, max_x, min_z, max_z, floor_y, ceiling, offset);
                        if has_core { generate_elevator_core(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, tipologia); }
                    }
                    Tipologia::Hospital => {
                        // Hospitais Genéricos e Postos de Saúde (Clínicas Candangas)
                        generate_corridor(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, bairro, tipologia);
                        generate_generic_hospital_layout(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, has_core);
                        if has_core { generate_elevator_core(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, tipologia); }
                    }
                    Tipologia::Corporativo => {
                        generate_corridor(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, bairro, tipologia);
                        generate_office_layout(editor, min_x, max_x, min_z, max_z, floor_y, ceiling, offset);
                        if has_core { generate_elevator_core(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, tipologia); }
                    }
                    Tipologia::MetroSubterraneo | Tipologia::MetroElevado | Tipologia::Religioso => {}
                    Tipologia::Residencial | Tipologia::Comercial | Tipologia::Generico => {
                        match bairro {
                            ...
                            Bairro::SQN => {
                                if i == 0 && has_core {
                                    generate_elevator_core(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, tipologia);
                                    continue;
                                }
                                generate_corridor(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, bairro, tipologia);
                                if has_core { generate_elevator_core(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, tipologia); }
                            ...
                            Bairro::Guara | Bairro::Samambaia | Bairro::Condominio => {
                                if height <= 12 && tipologia == Tipologia::Residencial {
                                    generate_residential_layout(editor, min_x, max_x, min_z, max_z, floor_y, ceiling, offset, bairro, total_floors);
                                } else {
                                    generate_corridor(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, bairro, tipologia);
                                    if has_core { generate_elevator_core(editor, min_x, max_x, min_z, max_z, floor_y + 1, ceiling, offset, tipologia); }
                                }
