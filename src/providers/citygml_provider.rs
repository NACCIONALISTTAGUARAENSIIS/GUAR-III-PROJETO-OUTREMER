...
impl CityGmlProvider {
    ...
    fn compute_element_aabb(
        ...
        if points_3d.is_empty() { return None; }

        let mut min_mc_x = i32::MAX; let mut max_mc_x = i32::MIN;
        let mut min_mc_z = i32::MAX; let mut max_mc_z = i32::MIN;
        let mut min_mc_y = f64::MAX; let mut max_mc_y = f64::MIN; // Elev

        for &(x, y, z) in points_3d {
            if let Ok((lon, lat)) = proj.convert((x, y)) {
                if let Ok(llpoint) = LLPoint::new(lat, lon) {
                    let mc_pt = transformer.transform_point(llpoint);
                    if mc_pt.x < min_mc_x { min_mc_x = mc_pt.x; }
                    if mc_pt.x > max_mc_x { max_mc_x = mc_pt.x; }
                    if mc_pt.z < min_mc_z { min_mc_z = mc_pt.z; }
                    if mc_pt.z > max_mc_z { max_mc_z = mc_pt.z; }
                    if z < min_mc_y { min_mc_y = z; }
                    if z > max_mc_y { max_mc_y = z; }
                }
            }
        }
            ...
            match reader.read_event_into(&mut buf) {
                ...
                Ok(Event::Text(e)) => {
                    if capture_text {
                        ...
                        if current_surface == GmlSurfaceType::Window || current_surface == GmlSurfaceType::Door {
                ...
                Ok(Event::End(ref e)) => {
                    ...
                    if name_str.contains("posList") || name_str.contains("pos") {
                        ...
                        if let Some((min_x, max_x, min_y, max_y, min_z, max_z)) = Self::compute_element_aabb(
                            ...
                            building_tags.insert(format!("lod3:window_{}:min_x", window_count), min_x.to_string());
                            building_tags.insert(format!("lod3:window_{}:max_x", window_count), max_x.to_string());
                            building_tags.insert(format!("lod3:window_{}:min_y", window_count), min_y.to_string());
                            building_tags.insert(format!("lod3:window_{}:max_y", window_count), max_y.to_string());
                            building_tags.insert(format!("lod3:window_{}:min_z", window_count), min_z.to_string());
                        ...
                        if let Some((min_x, max_x, min_y, max_y, min_z, max_z)) = Self::compute_element_aabb(
                            ...
                            building_tags.insert(format!("lod3:door_{}:min_x", door_count), min_x.to_string());
                            building_tags.insert(format!("lod3:door_{}:max_x", door_count), max_x.to_string());
                            building_tags.insert(format!("lod3:door_{}:min_y", door_count), min_y.to_string());
                            building_tags.insert(format!("lod3:door_{}:max_y", door_count), max_y.to_string());
                            building_tags.insert(format!("lod3:door_{}:min_z", door_count), min_z.to_string());
                            building_tags.insert(format!("lod3:door_{}:max_z", door_count), max_z.to_string());
                            door_count += 1;
                        }
                        current_surface = GmlSurfaceType::Wall;
                        current_surface_points.clear();
                        ...
                        if let Some((footprint, height, is_outside)) = Self::compute_building_footprint_and_height(
                            ...
                            if !is_outside {
                                ...
                                if window_count > 0 { building_tags.insert("lod3:windows".to_string(), window_count.to_string()); }
                                if door_count > 0 { building_tags.insert("lod3:doors".to_string(), door_count.to_string()); }
                ...
                Err(e) => {
                    eprintln!("[ALERTA] Erro de parsing SAX no CityGML na posição {}: {:?}", reader.buffer_position(), e);
                    break;
                }
